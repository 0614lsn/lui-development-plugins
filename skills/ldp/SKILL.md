...
L1 通用开发引擎：任何仓库、任何项目的「一个开发需求从立项到合并到收尾」的编排流程。LDP 只定义流程骨架与纪律，不绑定具体项目或 coding agent；项目专属信息（作用域、连接器、plan 闸门通道、收尾去处）由上层项目绑定声明（L2 模式见 `project-loop` skill），宿主能力由薄 adapter 映射，详见「环境绑定」节。
...
push（或本地合并）完成后按固定顺序执行 5a → 5b → 5c，三个子步骤都要收口，不得跳过或互相替代。每个子步骤先用环境绑定的去处；无绑定时用本插件自带的默认组件——5a 用 `knowledge`、5b 用 `self-evolution`、5c 用 `neat-freak`，LDP 独立使用也能完整执行。
...
- **去处与格式**：按 `knowledge` skill 的知识库规范执行（默认目标仓 `docs/knowledge/YYYY-MM-DD-<topic>.md` 并在同目录 `INDEX.md` 登记）；项目绑定可指定其它去处与索引。
...
- **去处与格式**：按 `self-evolution` skill 的记录规范执行（默认目标仓 `docs/evolution-log/YYYY-MM-DD-<topic>.md` + 声明的 frontmatter）；项目绑定可指定其它去处。
...
- 未闭合项必须落进跨会话登记入口（默认按 `knowledge` skill 的路由「悬而未决」约定；无路由文件时落 5a 的 knowledge 条目并注明未闭合），不停留在汇报文本里。
- **执行件**：默认由 `neat-freak` 组织六面汇报与清场状态机；无审计件可用时使用轻量兜底——对 README / docs 做对齐检查、清点会话残留与工作区临时物，如实标记证据和缺口，不编造运行态或外部系统结果。
...
本插件自带的可选组件（均不新增 LDP 阶段、不接管状态机、不复制角色合同）：

- `leader`：阶段 0–1 的规划与 goal 交接件，仅当需求明确要走 goal / 无人值守执行
  时挂载；未挂载时照常直接走 LDP。
- `knowledge`：阶段 5a 知识沉淀的默认去处，兼跨会话记忆（TODO / 路由文件）规范。
- `self-evolution`：阶段 5b 自进化复盘的默认记录件。
...
LDP 独立使用（无上层绑定）时，直接在当前宿主人机交互界面呈现 plan 并等待明确裁决，阶段 5 按各子步骤声明的默认组件与去处执行，其余阶段与纪律不变。
...
5. 阶段 4：自检四条件——全满足则按预授权合并 push；若 C2 留下一条 INCONCLUSIVE，则停下升级人工，用户要求调整就回阶段 3 形成新 Candidate。
